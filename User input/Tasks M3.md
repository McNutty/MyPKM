...
- Tests:
  - 1. Relationship labels render as card-style elements - OK!
  - 2. Relationship label editing (double-click card on line) - OK!
...
- Tests:
  ...
  - 6. Move a card (reparent) -> its relationships follow - OK!
  - 7. Delete a card with relationships -> relationships silently removed - OK!
  - 8. Relationships persist across reload - OK!
  - 9. Draggable relationship cards with curved arrows - OK!
  - 10. Relationship card position persists across reload - OK!
...
- Tests:
  - 1. Drag source/target card -> label moves proportionally, curve shape preserved - OK!
  - 2. Label positions persist after card drag + reload - OK!
  - 3. New relationship (default midpoint label) -> drag source, label stays at midpoint - OK!
...
- Tests:
  - 1. Move destination card to opposite side of source -> arrow emerges from correct edge - OK!
  - 2. Two relationships between same cards, labels opposite directions -> different edge exit/entry points - OK!
...
- Tests:
  - 1. Drag source/target so label passes near other card -> smooth movement without sticking - OK!
  - 2. Move parent card with children that have curved relationship arrows -> labels move correctly - OK!
...
- Tests:
  - 1. Edit relationship label, click outside -> text saves on blur (not just Enter) - OK!
...
- Tests:
  - 1. Hover nested cards -> only topmost card shows connection anchor points - OK!
...
- Tests:
  - 1. Default (never-dragged) labels render at midpoint of straight line -- same as before - OK!
  - 2. Perpendicular drag bends the curve -- same feel as before - OK!
  - 3. Parallel drag slides label along curve without changing curvature
  ...
  - 4. Labels near endpoints -- NO curve warping - OK!
    1. And it looks GLORIOUS!
  - 5. Two relationships between same cards -- independently positioned, no interference - OK!
  - 6. Double-click to edit opens at visual (on-curve) position - OK!
  - 7. Label stays under mouse during drag on curved arrows (no drift) - OK!
...
- Tests:
  - 1. Select an arrow, drag the source handle to a different card -> relationship rewires - OK!
  - 2. Select an arrow, drag the target handle to a different card -> relationship rewires - OK!
  - 3. Re-attach cancels when released on empty canvas - OK!
  - 4. Re-attach rejects self-loops (source === target) - OK!
...
- Tests:
  - 1. App auto-fits to show all cards on startup - OK!
...
- Tests:
  - 1. Hold Space to pan (works over cards too, not just empty canvas) - OK!
  - 2. Cursor changes to grab/grabbing during space panning (including over cards and connection handles) - OK!
  - 3. Hint overlay shows all shortcuts (Space, C, double-click) - OK!
...
- Tests:
  - 1. Press C to create a new card at cursor position - OK!
  - 2. C shortcut doesn't fire while editing text - OK!
...
- Tests:
  - 1. Nest a card into a parent (causing auto-expand), reload -> parent keeps its expanded size - OK!
  - 2. Manually resize a child card (make it bigger), reload -> parent still contains the child (no sticking out) - OK!
...
- Tests:
  - 1. Delete leaf card (no parts) -> immediate deletion, no dialog - OK!
  - 2. Delete card with parts -> dialog shows correct part count, Cancel leaves everything intact - OK!
  - 3. Delete All in dialog -> removes card, all descendants, and their relationships - OK!
  ...
  - 4. Deleting a card with parts does NOT briefly remove it then snap it back (old bug) - OK!
...
- Tests:
  - 1. F key on selected relationship -> flips direction (arrowhead reverses) - OK
...
- Tests:
  - 1. No `[db]` prefix in any error message reaching the user - OK as far as I can tell.
  - 2. "Containers" replaced with DSRP terms in code comments - I'll trust you on this one.
...
- Tests:
  - 1. Resize only triggers from lower-right corner, not full right/bottom border
...
- Tests:
  - 1. Shift+Scroll pans horizontally (up=left, down=right) - OK!
...
- Tests:
  - 1. Shift+Drag a card into a sibling -> sibling gets pushed in the drag direction - OK!
  - 2. Shift+Drag a card into a sibling directly on the canvas - OK!
  - 3. Pushing cascades: pushed card pushes further cards it collides with (including parent siblings) - OK!
  - 4. Pushed cards stop at left/top boundary of parent (or canvas edge for root cards) - OK!
  - 5. Dragged card overlaps when a pushed card hits a wall (accepted behavior for now)
  - 6. Parent extends right/down when pushed cards reach the border (immediate parent size remembered) - OK!
  - 7. Release Shift mid-drag -> pushing stops, normal drag resumes - OK!
  - 8. Press Shift mid-drag -> pushing activates without jump/snap - OK!
  - 9. Pushing mode does not interfere with Shift+Scroll horizontal panning - OK!
  - 10. Regular drag (no Shift) still works exactly as before - OK!
  - 11. All pushed card positions persist after mouse-up and reload - OK!
...
- Tests:
  - 1. Double-click parent card -> children shift to equal margins on all sides, parent shrinks to fit - OK!
  - 2. Double-click parent card -> size floor (minWidth/minHeight) is cleared - OK!
  - 3. Double-click parent card -> relative positions of children to each other are preserved - OK!
  - 4. Drop a card onto an empty card -> fit-to-contents applied automatically - OK!
  - 5. Double-click leaf card -> still resets to default size (no regression) - OK!
...
- Tests:
  - 1. Connection handles still appear on hover when no gesture is active (no regression) - OK!
...
- Tests:
  - 1. Relationship label positions persist correctly after dragging cards with relationships - OK!
